<x-layouts.sbadmin>
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            ...
            {{-- <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Daftar inventaris sarana per unit fisik dengan filter cepat.</p> --}}
        </div>
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.sarana.create') }}" class="btn-primary">
            ...
            <details class="relative group">
                ...
                <div class="absolute right-0 z-20 w-56 p-2 mt-2 bg-white border shadow-xl rounded-xl border-slate-200 dark:border-white/10 dark:bg-slate-900">
                    <button
                        ...
                        disabled
                        onclick="window.SarprasConfirm.open({
                            title: 'Konfirmasi Hapus Sarana',
                            message: 'Apakah Anda yakin ingin menghapus sarana yang terpilih? Sarana yang memiliki riwayat transaksi/aktivitas akan otomatis diarsipkan.',
                            confirmLabel: 'Ya, Hapus',
                            variant: 'danger',
                            onConfirm: () => document.getElementById('bulk-delete-form').submit()
                        })"
                    ...
                    <form
                        id="destroy-all-form"
                        method="POST"
                        action="{{ route('admin.sarana.destroy-all') }}"
                        class="mt-1"
                    >
                        @csrf
                        @method('DELETE')
                        <button type="button"
                                onclick="window.SarprasConfirm.open({
                                    title: 'Konfirmasi Hapus Semua Sarana',
                                    message: 'Apakah Anda yakin ingin menghapus semua sarana? Tindakan ini tidak dapat dibatalkan, dan sarana yang memiliki riwayat akan diarsipkan.',
                                    confirmLabel: 'Ya, Hapus Semua',
                                    variant: 'danger',
                                    onConfirm: () => document.getElementById('destroy-all-form').submit()
                                })"
                                class="flex items-center justify-between w-full px-3 py-2 text-sm font-semibold text-left transition rounded-lg text-rose-700 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-500/10">
    ...
    <section class="panel">
        <form method="GET" action="{{ route('admin.sarana.index') }}" class="filter-grid">
            ...
            <div>
                <label class="block mb-1 text-sm font-medium text-slate-700 dark:text-slate-200" for="status_sarana">Status</label>
                <select id="status_sarana" name="status_sarana" class="w-full text-sm bg-white shadow-sm rounded-xl border-slate-300 text-slate-800 focus:border-blue-500 focus:ring-blue-500/30 dark:border-white/15 dark:bg-slate-900/60 dark:text-slate-100 dark:focus:border-cyan-400 dark:focus:ring-cyan-400/40">
                    <option value="">Semua status</option>
                    @foreach ($statusList as $status)
                        <option value="{{ $status }}" @selected((string) $filters['status_sarana'] === (string) $status)>
                            {{ $status }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('admin.sarana.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold transition bg-white border rounded-lg border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-white/15 dark:bg-slate-800 dark:text-slate-300">
    ...
    <section class="mt-5 panel">
        ...
        <form id="bulk-delete-form" method="POST" action="{{ route('admin.sarana.destroy-selected') }}">
            @csrf
            @method('DELETE')
                    ...
                    <thead class="bg-slate-50 dark:bg-white/[0.04]">
                        <tr>
                            <th class="px-4 py-3 text-left">
                                <input id="select-all-sarana" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500/40">
                            </th>
                            ...
                            <tr>
                                <td class="px-4 py-3">
                                    <input
                                        ...
                                        name="sarana_ids[]"
                                        value="{{ $item->id }}"
                                        class="w-4 h-4 text-blue-600 rounded sarana-select-item border-slate-300 focus:ring-blue-500/40"
                                    >
                                </td>
                                <td class="px-4 py-3 font-mono text-xs whitespace-nowrap text-slate-500 dark:text-slate-400">{{ $item->kode_sarana }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold
                                        {{ $item->status_sarana === 'AKTIF' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-200' : 'bg-slate-200 text-slate-700 dark:bg-slate-600/30 dark:text-slate-200' }}">
                                        {{ $item->status_sarana }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-1.5">
                                        <a href="{{ route('admin.sarana.show', $item) }}"
                                        ...
                                        <a href="{{ route('admin.sarana.edit', $item) }}"
                                         ...
                                         <form
                                             ...
                                             action="{{ route('admin.sarana.destroy', $item) }}"
                                             method="POST"
                                             class="inline"
                                         >
                                             @csrf
                                             ...
                                             <button type="button"
                                                     onclick="window.SarprasConfirm.open({
                                                         title: 'Konfirmasi Hapus Sarana',
                                                         message: 'Apakah Anda yakin ingin menghapus sarana \'{{ $item->nama_sarana }}\'? Sarana yang memiliki riwayat aktivitas/perbaikan akan diarsipkan.',
                                                         confirmLabel: 'Ya, Hapus',
                                                         variant: 'danger',
                                                         onConfirm: () => document.getElementById('delete-form-{{ $item->id }}').submit()
                                                     })"
    ...
    <script>
        (function () {
            const selectAll = document.getElementById('select-all-sarana');
            const rowChecks = Array.from(document.querySelectorAll('.sarana-select-item'));
