<x-layouts.sbadmin>
    ...
    <div class="grid gap-5 lg:grid-cols-12">
        <section class="panel lg:col-span-5">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Tambah Ruangan</h2>
            <form method="POST" action="{{ route('admin.master.ruangan.store') }}" class="mt-4 space-y-4">
                @csrf
                <div>
        ...
        <section class="panel lg:col-span-7">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Daftar Ruangan</h2>
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-white/10 text-sm">
                    <thead class="bg-slate-50 dark:bg-white/[0.04]">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Ruangan</th>
                            <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Kode</th>
                            <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Gedung</th>
                            <th class="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Lantai</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/5 bg-transparent">
                        @forelse ($ruangan as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-slate-600 dark:text-slate-300"><span class="rounded-lg bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200">{{ $item->kode_ruangan }}</span></td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-sm text-slate-500 dark:text-slate-400">Belum ada data ruangan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
