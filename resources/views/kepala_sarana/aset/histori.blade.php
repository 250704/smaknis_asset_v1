<x-layouts.sbadmin>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="page-title">Histori Aset</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Riwayat kondisi kerusakan dan tindak lanjut aset.</p>
        </div>
        <a href="{{ route('kepala_sarana.aset.index') }}" class="btn-secondary">Kembali ke Data Aset</a>
    </div>

    <section class="panel">
        <form method="GET" action="{{ route('kepala_sarana.aset.histori') }}" class="grid gap-3 md:grid-cols-12">
            ...
            <div class="md:col-span-12 flex gap-2">
                ...
                <a href="{{ route('kepala_sarana.aset.histori') }}" class="btn-secondary">Reset</a>
            </div>
        </form>
    </section>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-white/10">
            <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-white/10">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                    @forelse ($histori as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                                <p class="font-mono text-xs">{{ $item->aset?->kode_aset ?? '-' }}</p>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-6 text-center text-slate-500 dark:text-slate-400">Belum ada histori kondisi aset.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
