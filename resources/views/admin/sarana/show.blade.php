<x-layouts.sbadmin>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.cetak-qr.index', ['q' => $sarana->kode_sarana]) }}" class="btn-secondary">Cetak QR</a>
            <a href="{{ route('admin.sarana.edit', $sarana) }}" class="btn-muted">Edit</a>
            <a href="{{ route('admin.sarana.index') }}" class="btn-secondary">Kembali</a>
        </div>
    </div>
        ...
        <section class="panel lg:col-span-7">
            ...
            <dl class="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <dd>
                        <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold
                            {{ $sarana->kondisi_terkini === 'BAIK' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200' : 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200' }}">
                ...
                <div>
                    ...
                    <dd>
                        <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold
                            {{ $sarana->status_sarana === 'AKTIF' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-200' : 'bg-slate-200 text-slate-700 dark:bg-slate-600/30 dark:text-slate-200' }}">
    ...
    <script>
        (function () {
            ...
            const namasarana = @json($sarana->nama_sarana);
            const lokasisarana = @json(($sarana->ruangan?->nama_ruangan ?? '-') . ' - ' . ($sarana->ruangan?->gedung?->nama_gedung ?? '-'));
            ...
            printBtn?.addEventListener('click', function () {
                ...
                const html = `
                    ...
                    <body>
                        <div class="label">
                            <img src="${qrSrc}" alt="QR ${kodesarana}">
                            <p class="code">${kodesarana}</p>
                            <p>${namasarana}</p>
                            <p>${lokasisarana}</p>
                        </div>
                    </body>
            ...
            downloadBtn?.addEventListener('click', function () {
                ...
                link.href = qrSrc;
                link.download = `QR-${kodesarana}.png`;
